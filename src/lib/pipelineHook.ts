...
function extractTableData(tableNode: any): Assertion[] {
  ...
  try {
    // 1. Check if using new format with attrs.rows
    ...
    const rows = (table as any).rows || table.content || [];

    // Process all rows
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      
      // Handle simplified row format: { row: [col1, col2, col3, col4], attrs: { disabled: false } }
      if (Array.isArray(row.row)) {
        if (row.attrs?.disabled === true) continue;
        const [description, field, operator, expectedValue] = row.row;
        if (!description && !field && !operator && !expectedValue) continue;

        assertions.push({
          description: String(description || '').trim(),
          field: String(field || '').trim(),
          operator: String(operator || 'equals').trim() || 'equals',
      ...
      // Handle standard ProseMirror tableRow
      ...
      const description = extractTextFromNode(cells[0]).trim();
      const field = extractTextFromNode(cells[1]).trim();
      const operator = extractTextFromNode(cells[2]).trim();
      const expectedValue = extractTextFromNode(cells[3]).trim();
...
export async function postProcessAssertionsHook(context: any): Promise<void> {
  ...
  try {
    ...
    const replaceVars = (typeof window !== 'undefined' ? (window as any) : undefined)?.electron?.env?.replaceVariables;
    if (replaceVars) {
